...
| I10 | 通知状态和调试入口分散 | Settings、Memory Intelligence、Debug Remote Push、Notification Background、Job Queue 各自展示一部分状态 | completed：用户 Settings 只保留 `NotificationManagementView` 偏好设置；队列、历史、去重、错误、Push diagnostics 收敛到唯一 Debug `DebugRuntimeOperationsView` | `NotificationManagementEventTests` + `RuntimeOperationsCoordinatorTests` + app build |
| I11 | 后台触发和后台状态分散 | AppDelegate、Root、Home、pipeline、BGTask、silent push、URLSession 分散触发恢复、job、通知、push sync，难以判断一次后台运行做了什么 | completed：统一到 `BackgroundOperationOrchestrator`，所有入口记录 `BackgroundOperationRun/Event`；Background 仅依赖 `BackgroundJobProcessing/Recovering/QuestionPreparing/ReminderRouting/PushRegistrationSyncing` ports，Intelligence/Notification/Push 业务实现留在各自域；Debug 只通过 `RuntimeOperationsCoordinator` 触发正式 orchestrator | background rg audit + app build + Runtime Operations debug surface |
