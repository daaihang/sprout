...
| 卡片排布 | 可用 | 新建页和详情页使用同一套 arrangement 语义：contentDensity、order、stack/group、贴纸挂点、rotation/nudge/zIndex；瀑布流 frame 运行时派生。 |
...
- `MemoryCardObjectMetrics` 只在渲染时从 content kind + density + column width 派生物件尺寸和文本详略，不持久化。
