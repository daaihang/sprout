...
| --- | --- | --- |
| Analyze | `AnalysisRecordPayloadBuilder.swift`, `AnalysisExecutor.swift`, `server/internal/ai/types.go` | 只带当前记忆、附件、最多 20 个轻量 known entities，没有历史 context pack |
...
| Arc/Reflection | `AnalysisExecutor.swift`, `MoryAPIClient.swift` | Arc/Reflection 多为后处理，Reflection 请求只有 linked id 而非 evidence |
| Background | `BackgroundOperationOrchestrator.swift`, `BackgroundTaskCoordinator.swift`, `BackgroundURLSessionInfrastructure.swift` | 只负责什么时候跑、跑了什么、状态如何、是否重试；不持有分析/通知/Push 业务实现 |
| Notifications | `NotificationOrchestrator.swift`, `NotificationDeliveryRouter.swift`, `LocalNotificationScheduler.swift` | 只负责通知生成、去重、策略、路由；远程投递通过 Push 窄口 |
| Push | `RemotePushSyncService.swift`, `PushDeviceRegistrationStore`, `server/internal/push` | 负责 APNs token、远程注册、delivery queue、writeback、APNs retry/pacing |
...
| [07 Cloud Contracts v7](07_cloud_contracts_v7.md) | 定义 `/api/analyze`、context-aware reflection、proposal-first AI 输出 |
