...
| Background/Notifications | `BackgroundOperationOrchestrator.swift`, `BackgroundTaskCoordinator.swift`, `NotificationOrchestrator.swift`, `NotificationDeliveryRouter.swift`, `LocalNotificationScheduler.swift` | 后台触发已收敛到 Background 域；通知仍由 Notification 域作为 trigger -> dedupe -> policy -> routing 单入口 |
