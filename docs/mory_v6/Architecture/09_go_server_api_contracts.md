...
## 9. Notification Generation Boundary

Mory no longer exposes a cloud endpoint that suggests notification intents.
All notification candidates are generated by the iOS `NotificationOrchestrator`,
then delivered locally or enqueued to `/api/push/enqueue`. The server remains a
push delivery queue and must not create product notification intent semantics.
