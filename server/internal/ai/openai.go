...
func (p *OpenAICompatibleProvider) runReflection(
	...
	payload := openAIChatRequest{
		...
		Temperature:    0.2,
		Messages: []openAIMessage{
			{Role: "system", Content: buildReflectionSystemPrompt(mode)},
			{Role: "user", Content: userPrompt},
		},
	}
	...
	resp, err := doRequestWithRetry(ctx, p.client, httpReq, p.maxRetries, p.backoff)
	if err != nil {
		p.logger.Error("openai-compatible reflection request failed", "provider", p.Name(), "mode", mode, "error", err)
	...
	if resp.StatusCode >= 300 {
		p.logger.Error(
			"openai-compatible reflection response failed",
			"status", resp.StatusCode,
			"provider", p.Name(),
			"mode", mode,
			"body", truncateBody(responseBody, 2048),
	...
	if err := json.Unmarshal(responseBody, &decoded); err != nil {
		p.logger.Error(
			"openai-compatible reflection decode failed",
			"provider", p.Name(),
			"mode", mode,
			"status", resp.StatusCode,
			"body", truncateBody(responseBody, 2048),
