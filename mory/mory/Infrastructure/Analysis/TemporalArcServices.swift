...
struct TemporalArcPromoter {
    func promote(
        ...
        let interpretations = candidate.recordIDs.compactMap { analysisIndex[$0]?.emotionInterpretation }
        let dominantEmotion = mostFrequentValue(in: interpretations)
...
struct TemporalArcMergeEngine {
    ...
    func previewMerge(base: TemporalArc, candidate: TemporalArc) -> TemporalArcMergePreview? {
        ...
        return TemporalArcMergePreview(sourceArcID: base.id, candidateArcID: candidate.id, overlapScore: overlap)
    }
    ...
    func merge(base: TemporalArc, candidate: TemporalArc, mergedAt: Date = .now) -> TemporalArc {
        ...
        let mergedTitle = buildMergedTitle(base: base, candidate: candidate, dominantTheme: dominantTheme, dominantEntity: dominantEntity)
    ...
    func overlapScore(base: TemporalArc, candidate: TemporalArc) -> Double {
        ...
        return recordOverlap * 0.40 + themeOverlap * 0.25 + entityOverlap * 0.20 + timeOverlap * 0.15
    }
    ...
    private func buildMergedTitle(base: TemporalArc, candidate: TemporalArc, dominantTheme: String?, dominantEntity: String?) -> String {
        if let dominantTheme, let dominantEntity { return "\(dominantTheme) around \(dominantEntity)" }
        if let dominantTheme { return dominantTheme }
        if let dominantEntity { return dominantEntity }
