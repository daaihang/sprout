...
nonisolated struct AffectSnapshotMapper: Sendable {
    ...
    func draftFromJournalingStateOfMind(
        ...
        ) ?? AffectSnapshotDraft(rawInput: label)
        base.sources = [.journalSuggestionStateOfMind]
        base.valence = valence ?? base.valence
        base.arousal = arousal ?? base.arousal
        base.dominance = dominance ?? base.dominance
        base.intensity = intensity ?? base.intensity
        base.confidence = max(base.confidence ?? 0.75, 0.8)
        base.evidenceSummary = "Journaling Suggestion StateOfMind: \(label)"
    ...
    private func labels(for normalized: String) -> [AffectLabel] {
        ...
        if contains(normalized, ["irritated", "annoyed", "烦", "烦躁", "恼火"]) { labels.append(.irritated) }
    ...
    private func vector(
        ...
        if labels.contains(.irritated) || labels.contains(.stressed) || toneHints.contains(.serious) {
            return (-0.7, 0.8, 0.25, 0.72)
        }
        if labels.contains(.anxious) || labels.contains(.tense) || labels.contains(.overwhelmed) {
            return (-0.55, 0.82, 0.25, 0.75)
        }
        if labels.contains(.tired) || labels.contains(.sad) || labels.contains(.lonely) || labels.contains(.numb) {
            return (-0.5, 0.25, 0.25, 0.5)
        }
        if labels.contains(.calm) || labels.contains(.relieved) || labels.contains(.warm) || labels.contains(.grateful) {
            return (0.65, 0.25, 0.72, 0.45)
        }
        if labels.contains(.excited) || labels.contains(.inspired) || labels.contains(.proud) || labels.contains(.curious) {
