...
struct SproutTemporalArcService {
    ...
    func rebuildAcceptedArcs(
        ...
        let candidates = candidateBuilder.buildCandidates(
            ...
            maxCandidates: limit * 2
        )

        return candidates
            .filter { candidate in
                candidate.recordIDs.count >= 2 || candidate.clusterStrength >= 0.48
            }
            .prefix(limit)
            .enumerated()
...
private struct TemporalArcCandidateBuilder {
    ...
    private func buildContext(
        ...
        let entityNames = linkedEntityIDs
            .compactMap { entityIndex[$0]?.displayName }
            .sorted()
        let themeLabels = Array(Set(analysis?.tags ?? [])).sorted()
    ...
    private func bestAdjacentPair(
        ...
        contexts
            ...
            .sorted { lhs, rhs in
                if lhs.1 == rhs.1 {
                    return lhs.0.record.updatedAt < rhs.0.record.updatedAt
                }
                return lhs.1 > rhs.1
            }
            .first?
    ...
    private func buildTextBag(recordText: String, artifactTexts: [String]) -> Set<String> {
        let corpus = ([recordText] + artifactTexts)
            .joined(separator: " ")
            .lowercased()
    ...
    private func buildTitleHint(themeLabels: [String], entityNames: [String]) -> String {
        ...
        if !themePart.isEmpty && !entityPart.isEmpty {
            return "\(themePart) around \(entityPart)"
        }
...
private struct TemporalArcPromoter {
    func promote(
